...
void Form::signForm(Bureaucrat &temp) {
	if (this->isSigned){
		std::cout<<temp.getName()<<" couldn’t sign "<<this->_name<<" because she's already signed."<<std::endl;
		return ;
	}
	if (temp.getGrade() > this->gradeRequired){
		std::cout<<temp.getName()<<" couldn’t sign "<<this->_name<<" because too small a rank."<<std::endl;
		return ;
	}
	this->isSigned = true;
